...
describe("dictionary search", () => {
  ...
  it("deleting a word from search results returns to the filtered list", async () => {
    // Arrange
    ...
    // Act
    const after = await tap(results.messageId, `dict:confirm-delete:${dictionaryId}:${kotvaId}:1`);

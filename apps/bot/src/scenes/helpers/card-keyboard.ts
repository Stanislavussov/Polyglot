...
async function resolveCardKeyboardOptions(
  ...
  return {
    ...
    // Under review the deck's own Remove takes the word out and moves on to the next
    // card; a second one here would remove it and leave the deck standing on its ghost.
    hideSaveSlot: entry.reviewCard !== undefined,

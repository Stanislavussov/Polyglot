...
`packages/adapters/notifications/src/index.ts` is empty (`export {}`). Everything needs to be implemented.
...
```
packages/adapters/notifications/src/
├── index.ts                    # Re-exports: startScheduler, stopScheduler
├── types.ts                    # SendFn, NotificationPayload, etc.
├── scheduler.ts                # node-cron setup, checkAndSend logic
└── notification.service.ts     # getUsersForNotification, buildPayload, pickWord
```

...
- Translate mode asks the user to choose the source language only when language detection has actionable ambiguity, such as dictionary evidence for the same spelling in multiple candidate languages; weak shared-script ambiguity alone must not interrupt normal translation.
- Translate mode handles core `needs_clarification` decisions with Telegram clarification buttons for ambiguity options, core-provided source-language choices, typo corrections, translate-as-written, and context entry; source-language fallback buttons are shown only for `source_language` ambiguity, typo corrections can switch to the corrected word's `langCode`, and context replies are stored in session and retried as `topic`.
- Translation timing records use `decision.quality.modelId` for accepted translations so model-routing telemetry stores the actual generation model, not only the pre-routing default.

...
- **Ambiguous homograph guard**: `WiktionaryStrategy` checks every unique candidate language and only resolves a language when exactly one dictionary matches. Multi-language spellings such as English/German `fast` return `undefined`, skip the AI fallback, and remain available for caller-side source-language clarification. The AI fallback prompt also accepts `ambiguous` instead of forcing a language guess.

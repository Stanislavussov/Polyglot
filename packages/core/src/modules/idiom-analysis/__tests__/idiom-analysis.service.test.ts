...
import type { IdiomAnalysisResult, GenerateObjectFn } from '../types.js';
...
describe('analyzeIdiom', () => {
  ...
  it('propagates errors from generateObjectFn', async () => {
    ...
    await expect(
      analyzeIdiom(mockInput, mockGenerateObjectFn as unknown as GenerateObjectFn)
...
describe('analyzeIdiomBatch', () => {
  ...
  it('processes multiple inputs sequentially', async () => {
    ...
    const results = await analyzeIdiomBatch(
      inputs,
      mockGenerateObjectFn as unknown as GenerateObjectFn
    );
  ...
  it('propagates first error in batch', async () => {
    ...
    await expect(
      analyzeIdiomBatch(inputs, mockGenerateObjectFn as unknown as GenerateObjectFn)
    ).rejects.toThrow('AI service error');
  });
});
...
describe('needsIdiomReview', () => {
  ...
  it('returns true for LITERAL_BUT_UNNATURAL classification', async () => {
    ...
    const result = await needsIdiomReview(
      mockInput,
      mockGenerateObjectFn as unknown as GenerateObjectFn
    );

...
- Include top-level "sourceUsage" for the source word "${text}":
  ...
  * "recallHint": ONE short sentence in ${nativeLangName ?? "the user's native language"} shown on a flashcard BEFORE the answer, to nudge recall without giving it away. Hint at register, connotation, typical setting, or who says it (e.g. "Colloquial and a little disapproving; said about people"). NEVER translate, define, or paraphrase "${text}", and never name a synonym or the translation in any language. Use null when no such hint is possible.${traitLine}`;
}

/**
 * The authoritative one-sense rule. Each target language is generated by its own
 * AI call, so a polysemous word ("wasted" = drunk / squandered) otherwise yields
 * a card whose blocks disagree — a Czech block about wasted time under a 🥴 emoji
 * and drunk-sense examples. The sense the metadata call settled on is restated
 * here so every parallel call renders the same one.
 */
function normalizeSenseAnchor(senseAnchor?: string): string | undefined {
  const trimmed = senseAnchor?.trim();
  return trimmed !== undefined && trimmed.length > 0 ? trimmed : undefined;
}

function buildSenseAnchorRule(text: string, senseAnchor: string): string {
  return `

SENSE ANCHOR (authoritative): this card covers exactly ONE sense of "${text}" — ${senseAnchor}.
- In every target language, "text", its synonyms, examples, usageNote, and alternatives MUST all render THIS sense.
- Never switch to another sense of "${text}", even when a different sense is more common in the target language.
- When this sense has no single-word equivalent in a target language, use the closest natural wording for THIS sense — never a word for a different sense.`;
}
...
export function buildTranslationPrompt(request: TranslationRequest): string {
  ...
  const senseAnchor = isSentence ? undefined : normalizeSenseAnchor(request.senseAnchor);
  const senseAnchorHint = senseAnchor ? buildSenseAnchorRule(text, senseAnchor) : "";
  ...
  return `${intro}${topicHint}${negativeHint}${dictionaryHint}${senseAnchorHint}
...
${
  senseAnchor
    ? `- The alternatives are other ways to say THIS anchored sense — a different wording, register, or nuance. Never use an alternative to introduce a different sense of "${text}".`
...
export function buildMetadataPrompt(
  ...
  /**
   * Must mirror the `includeSenseAnchor` flag the metadata SCHEMA was built
   * with. Asking for a field a strict schema forbids is a provider rejection,
   * so the caller passes one boolean to both rather than each re-deriving it.
   */
  resolveSenseAnchor = request.inputType !== "sentence",
): string {
  const { text, sourceLang, targetLangs, nativeLang, topic, dictionaryContext, outputConfig, inputType } = request;
  const cfg = resolveConfig(outputConfig);
  const isSentence = inputType === "sentence";
  ...
  if (resolveSenseAnchor) {
    requestedFields.push("primarySense");
  }
  if (assessExistence) {
  ...
  const anchoredFields = [
    ...(cfg.includeEmoji ? ["the emoji"] : []),
    ...(nativeLangName && cfg.includeNativeMeaning ? ['"nativeMeaning"'] : []),
    ...(isLearningSource && !isSentence ? ['the "sourceUsage" examples'] : []),
  ];
  const senseChoiceRule = request.negativeConstraints
    ? `
  * Choose a sense NOT represented by the already-shown translations listed above — the user asked for a different meaning, and this descriptor is what makes the whole card switch to it.`
    : `
  * Choose the sense a learner most likely means${topic ? " — the user context hint above decides it" : ""}${dictionaryContext ? ", weighing the dictionary reference above" : ""}.`;
  const senseAnchorRule = !resolveSenseAnchor
    ? ""
    : `
- Include top-level "primarySense": a short descriptor, WRITTEN IN ENGLISH (3-10 words), of the single sense of "${text}" this card is about. It anchors every target language to one sense, so it must name the sense precisely enough to rule the others out — e.g. for English "wasted": "intoxicated by alcohol or drugs (slang)", NOT "wasted".${senseChoiceRule}${
        anchoredFields.length > 0 ? `\n  * ${anchoredFields.join(", ")} must describe THIS sense.` : ""
      }
  * Set it to null only when the input has no nameable sense at all.`;
...
export function buildMetadataStrictPrompt(
  ...
  const base = buildMetadataPrompt(request, assessExistence, resolveSenseAnchor);
  const errorFeedback = errors.map((e) => `  - ${e}`).join("\n");
  return `${base}

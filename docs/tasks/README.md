# Polyglot — Tasks

| #   | Task                                              | Status     |
| --- | ------------------------------------------------- | ---------- |
| 01  | [Init project with monorepo](./01-init-monorepo.md) | ✅ Done    |
| 02  | [Create DB schemas and push to DB](./02-db-schemas.md) | ✅ Done    |
| 03  | [First step on bot creation](./03-bot-setup.md)   | ✅ Done    |
| 04  | [AI Translation Pipeline](./04-ai-translation-pipeline.md) | ✅ Done   |
| 05  | [Structured Logging](./05-logging.md)             | ✅ Done    |
| 06  | [AI Token Optimization](./06-token-optimization.md) | 🔲 To Do  |
| 07  | [Partial Translation Regeneration](./07-partial-regeneration.md) | ✅ Done |
| 08  | [AI Model Fallback](./08-model-fallback.md)       | 🔲 To Do  |
| 09  | [Translation Session Loop](./09-translate-session-loop.md) | ✅ Done   |
| 10  | [Idiomatic & Proverb Equivalent Matching](./10-idiomatic-equivalents.md) | ✅ Done   |
| 11  | [Input Message Limits & Validation Config](./11-input-limits-config.md) | 🔲 To Do  |
| 12  | [Detect Literal vs Idiomatic Translation Nuances](./12-idiom-analysis.md) | 🟡 In Progress  |
| 13  | [Wiktionary JSONL Integration](./13-wiktionary-jsonl.md) | 🔲 To Do  |
| 14  | [Refactor Language Usage to `languages` Table](./14-language-table-refactor.md) | 🔲 To Do  |

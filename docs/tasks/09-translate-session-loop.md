...
- [x] Define a `UserMode` type (e.g. `"translate" | "idle"`) — extensible for future modes like `"mentor"`
- [x] Store the user's current active mode in the grammY session data (or equivalent state)
  - Default mode for new users / after `/start`: `"idle"` (or directly `"translate"` — TBD)
- [x] When the user sends `/translate`, set active mode to `"translate"` and confirm with a message
- [x] When the user sends another mode command (e.g., future `/mentor`), switch active mode accordingly
- [x] Non-mode commands (`/help`, `/start`, `/settings`) do **not** change the active mode
...
- [x] In locale files (`packages/core/src/modules/i18n/locales/en.json`, `ru.json`, `cs.json`):
...
- [x] Add a middleware / message handler that intercepts **all plain text messages** (non-command):
...
- [x] After translation result is shown with Save/Skip, the bot does **not** exit any scene — the user remains in translate mode. The next plain text message will trigger another translation automatically.

### Step 4: Refactor translate scene → translate mode handler

- [x] The current `translate.scene.ts` conversation-based scene should be refactored:
...
- [x] Extract translation handler logic into `apps/bot/src/scenes/helpers/translate-mode.helper.ts` if the file exceeds 100 lines

### Step 5: Write tests

- [x] Test: send `/translate` → mode is set, confirmation message shown
- [x] Test: send a word while in translate mode → translation pipeline runs, result shown
- [x] Test: send multiple words in succession → each triggers a translation (no re-entering `/translate`)
- [x] Test: send `/settings` while in translate mode → settings open, mode remains `"translate"`
- [x] Test: send a future mode command (mock `/mentor`) → mode switches away from `"translate"`
- [x] Test: send text while in `"idle"` mode → hint message shown
- [x] Test: Save/Skip callbacks work correctly within translate mode
...
- `packages/core/src/modules/i18n/types.ts` — added `translateModeOn`, `translateModeHint` keys ✅
- `packages/core/src/modules/i18n/locales/en.json` — added English translations ✅
- `packages/core/src/modules/i18n/locales/ru.json` — added Russian translations ✅
- `packages/core/src/modules/i18n/locales/cs.json` — added Czech translations ✅
- `apps/bot/src/types.ts` — added `UserMode` type, `SessionData` interface with `activeMode` field ✅
- `apps/bot/src/middlewares/mode-router.ts` — **new** — middleware that routes plain text to the active mode handler ✅
- `apps/bot/src/scenes/translate.scene.ts` — refactored from conversation-based scene to mode-based handler ✅
- `apps/bot/src/scenes/helpers/translate-mode.helper.ts` — **new** — handles translation text and Save/Skip callbacks ✅
- `apps/bot/src/index.ts` — added session middleware, callback handlers, mode router middleware ✅
- `apps/bot/src/__tests__/translate-mode.test.ts` — **new** — tests for persistent mode behavior ✅
...
- [x] Sending `/translate` activates translate mode and shows a confirmation message
- [x] While in translate mode, every plain text message is automatically treated as a word/phrase to translate — no need to re-enter `/translate`
- [x] After Save/Skip, the user remains in translate mode — the next text message triggers another translation
- [x] Sending a different mode command (e.g., future `/mentor`) switches the active mode away from translate
- [x] Non-mode commands (`/help`, `/settings`, `/start`) execute normally without changing the active mode
- [x] In `"idle"` mode (no mode selected), plain text shows a hint to pick a mode
- [x] The `UserMode` type is extensible — adding future modes requires only a new enum value + handler
- [x] All texts use i18n — no hardcoded strings
- [x] Translate handler stays within the 100-line limit (or helper is extracted)
- [x] All new and existing tests pass: `pnpm test`

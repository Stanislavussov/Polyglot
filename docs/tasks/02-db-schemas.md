...
- [ ] Install dependencies in `@polyglot/adapter-db`: `drizzle-orm`, `postgres` (or `pg`), `drizzle-kit`
- [ ] Create `packages/adapters/db/schema.ts` with all tables:
...
- [ ] Create `drizzle.config.ts` reading `DATABASE_URL` from env
- [ ] Create `packages/adapters/db/index.ts` with `getDb()` singleton (creates one connection pool)
- [ ] Generate initial migration: `pnpm drizzle-kit generate`
- [ ] Push schema to database: `pnpm drizzle-kit push` (or `migrate`)
- [ ] Verify tables exist in PostgreSQL (connect and inspect)
- [ ] Create repository stubs per `tech-reqs/14-agents.md`:
...
- [ ] Add basic repository tests (vitest, integration against test DB)

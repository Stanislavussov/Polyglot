# Task 84 — Onboarding reverse trial (a week of Pro, then a survivable free tier)
...
**Tier.** Two readings, and the second one won.

The first shipped: Plus, not Pro. "Every feature" hands out the two most expensive calls on the platform (TTS, mentor) to a cohort that has not yet shown it will return, leaves nothing above the tier the user was just given, and teaches them that pronunciation and voice input are part of the product before taking them away.

It was reversed to **Pro** because it made the card lie about itself. Buttons are badged with the glyph of the tier that sells them (⭐ Plus, 💎 Pro), so a Plus trial left 💎 on the first card a newcomer ever sees — the single screen the whole onboarding exists to make convincing. A newcomer forming their opinion in week one has to meet the product, not a tier of it; the cost of the two extra calls is bounded by the same seven days, and what is sold afterwards is the whole thing rather than the top slice. The upsell after the week is Pro itself, which is exactly what the user just lost.
...
So: a **seven-day Pro** reverse trial, announced honestly at the start, warned about before it ends, earnable by use, landing on a free tier that still works.
...
|---|---|---|
| Plan pointer | `pro` | `free` |
| Length | 7 days from reaching the demo screen, +3 days once if ≥ 10 words saved | — |
| Translations | unlimited | 30 / month (was 10) |
| Other meanings / clarify | ✅ | 🔒 |
| Mentor | ✅ (unmetered, as Pro) | 🔒 |
| YouTube video | ✅ | 🔒 (the one onboarding video stays) |
| Pronunciation / voice input | ✅ | 🔒 |

**Where the grant happens is part of the spec, not an implementation detail.** It runs on entry to the demo screen, one update before the first card can render, because the card resolves its badges from the plan pointer at render time: granted any later, the newcomer's first card carries locks for features they already hold. It is not granted earlier than that — a user who abandons on the language picker has used nothing, and the gift is once per account forever.

### 2.1 The four messages

1. **Grant**, written on entry to the demo screen and announced once on the closing screen, which reads the live trial back out of the ledger rather than granting it: names the tier, the seven days, what stays free afterwards, and the earn-more rule. Announced only when the account actually holds a trial.
...
A row also has to still speak for the user before anything is sent: `activate` supersedes a trial the moment the user *buys*, and telling a paying convert that their free week is over would aim the worst message in the product at the one cohort the whole feature exists to produce.
...
- **Transactional paired writes.** `grantOnboardingTrial` (row, then plan pointer) and `expireAndDowngrade` (status, then pointer) each write through two repositories, and the port layer has no cross-repository transaction. The row-first order is the safe one — the reverse could leave a user holding the paid tier with nothing to expire it — and a failed pointer write now retires its own row (`canceled`), which keeps the gift spent while making both sweeps skip it forever.
  ...
  - A pointer write that **commits and then throws** — a dropped connection or a client timeout after the server committed. The compensation then cancels a row whose pointer did take, leaving `subscription_plan = 'pro'` with no active row: the paid tier indefinitely, and `currentPlanPrice` falls back to the list price, so the upgrade screen has nothing left to offer and refuses every tap. Rarer than the any-failure case the compensation replaces, and the alternative outcome (loudly telling a never-trialled user their free week is over) is the more common one. Guarding it properly means re-reading the pointer before retiring the row, which widens `SubscriptionUserUpdater` — a deliberately narrow port — for a tail case.

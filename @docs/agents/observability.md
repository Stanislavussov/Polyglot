...
| Notifications / cron | `notification.sent`, `notification.delivery_log_failed`, `notification.interaction` (`deliveryId`, `kind`, `action` — a button tapped on a journaled notification), `notification.interaction_log_failed` (error), `notification.dictionary_exhausted`, `notification.preset.picked`/`.exhausted`/`.no_candidates`/`.unresolvable`, `nudge.*`, `retention.*` |
